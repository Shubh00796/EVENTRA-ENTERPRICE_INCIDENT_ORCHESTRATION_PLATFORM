...
public interface EventRepository extends JpaRepository<Events_EIOP, Long> {
    ...
    Page<Events_EIOP> findAllByStatus(EventStatus status, Pageable pageable);
    ...
    Page<Events_EIOP> findActiveEventsByType(EventType eventType, Pageable pageable);


    @Query("SELECT e FROM Events_EIOP e WHERE e.venueType = :venueType ")
    Page<Events_EIOP> findEventsByVenueType(VenueType venueType, Pageable pageable);
